class ProposalsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @proposal.save
        flash[:success] = "Thanks for your project !"
        format.html { redirect_to(profile_proposal_path(@proposal)) }
  ...
  def update
    ...
    respond_to do |format|
      if @proposal.update_attributes(params[:proposal])
        format.html { redirect_to(profile_proposal_path(@proposal), :notice => 'Proposal status was successfully updated.') }
        format.xml  { head :ok }
      else
